...
def mask_to_boundary(mask, dialtion_ratio=0.02):
    mask = np.asarray(mask, np.float32)
    h, w = mask.shape
    img_diag = np.sqrt(h ** 2 + w ** 2)
    dilation = int(round(dialtion_ratio * img_diag))
    if dilation < 1:
        dilation = 1
...
def boundary_iou(gt, dt, dilation_ratio=0.02):
    gt_boundary = mask_to_boundary(gt, dialtion_ratio=dilation_ratio)
    dt_boundary = mask_to_boundary(dt, dialtion_ratio=dilation_ratio)
    dt_boundary = (dt_boundary > 0.5).astype(np.float32)
...
def evaluate(pred_path, gt_path, verbose=True):
    ...
    headers = ['meanDic', 'meanIoU', 'wFm', 'Sm', 'meanEm', 'mae', 'maxEm', 'maxDic', 'maxIoU', 'meanSen', 'maxSen',
               'meanSpe',
               'maxSpe']  # ['meanDic', 'meanIoU', 'wFm', 'Sm', 'meanEm', 'mae', 'maxEm', 'maxDic', 'maxIoU', 'meanSen', 'maxSen', 'meanSpe', 'maxSpe']
    results = []
